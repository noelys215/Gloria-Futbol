...
export const footballApi = createApi({
	...
	endpoints: (builder) => ({
		getStandings: builder.query({
			query: (league) => createRequest(`/standings?league=${league}&season=2021`),
		}),
		getTopScorer: builder.query({
			query: (league) => createRequest(`/players/topscorers?league=${league}&season=2021`),
		}),
		getMostAssists: builder.query({
			query: (league) => createRequest(`/players/topassists?league=${league}&season=2021`),
		}),
		getMostRedCards: builder.query({
			query: (league) => createRequest(`/players/topredcards?league=${league}&season=2021`),
		}),
		getMostYellowCards: builder.query({
			query: (league) =>
				createRequest(`/players/topyellowcards?league=${league}&season=2021`),
		}),
		getFixtures: builder.query({
			query: ({ fixtureId, league, selectedDate = format(new Date(), 'yyyy-MM-dd') }) =>
				createRequest(`/fixtures?league=${league}&season=2021&date=${selectedDate}&timezone=America/New_York
			`),
		}),
